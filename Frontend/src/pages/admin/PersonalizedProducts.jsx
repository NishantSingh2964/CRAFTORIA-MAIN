...
const AdminPersonalizedProducts = () => {
  const { personalizedProducts, loading, fetchPersonalizedProducts, deletePersonalizedProduct } = usePersonalized();
  ...
  const categories = useMemo(() => {
    const unique = [...new Set(personalizedProducts.map((product) => product.category).filter(Boolean))];
    return ['All Categories', ...unique];
  }, [personalizedProducts]);

  const filteredProducts = useMemo(() => {
    return personalizedProducts.filter((product) => {
  ...
  }, [category, personalizedProducts, query, status]);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this personalized product?')) {
      await deletePersonalizedProduct(id);
    }
  };
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="font-serif text-[34px] font-black leading-tight text-[#171111]">Personalized Gifts</h1>
          </div>
          <p className="text-sm font-medium text-[#6c5c58]">Independent collection of custom-made products.</p>
        </div>
        <Link
          to="/admin/personalized-products/add"
        ...
        <div className="flex flex-col gap-4 border-b border-[#efe3df] p-5 xl:flex-row xl:items-center">
          ...
          <div className="grid gap-4 sm:grid-cols-3 xl:w-[610px]">
            ...
            <button
              ...
              className="inline-flex h-11 items-center justify-center gap-2 rounded-lg border border-[#e4d5cf] bg-white px-5 text-sm font-bold text-[#8d0000] transition hover:border-[#9a1515]"
            >
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1080px] text-left">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan="8" className="px-8 py-20 text-center font-serif text-xl text-[#8b7772]">Loading custom gifts...</td>
                </tr>
              ) : paginatedProducts.length === 0 ? (
                <tr>
                  <td colSpan="8" className="px-8 py-20 text-center text-sm font-medium text-[#8b7772]">No personalized products found.</td>
                </tr>
                ...
                return (
                  <tr key={product._id} className="border-b border-[#efe3df] transition hover:bg-[#fffaf7]">
                    ...
                    <td className="px-4 py-4">
                      <span className="inline-flex rounded-md bg-[#fff7f0] px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-[#8d0000] border border-[#f5e6d3]">
                        {product.personalizationType || 'Text'}
                      </span>
                    </td>
                    <td className="px-4 py-4 text-sm font-semibold text-[#171111]">{product.category}</td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-3">
                        <Link 
                          to={`/admin/personalized-products/edit/${product._id}`}
...
export default AdminPersonalizedProducts;
