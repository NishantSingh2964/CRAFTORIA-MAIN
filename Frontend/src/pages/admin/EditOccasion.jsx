...
import { ChevronDown, Loader2, Save, Upload, X, ArrowLeft } from 'lucide-react';
...
const EditOccasion = () => {
  ...
  const { occasions, updateOccasion, loading: contextLoading } = useOccasions();
    ...
    if (occasions.length > 0) {
      ...
      if (occasion) {
        setFormData({
          ...
          desc: occasion.desc || '',
          tag: occasion.tag || 'New Collection',
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await updateOccasion(id, submissionData);
    if (result.success) {
    ...
    <form onSubmit={handleSubmit} className="space-y-7">
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
        <div className="space-y-4">
          <Link to="/admin/occasions" className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-widest text-[#8d0000] hover:underline">
            <ArrowLeft className="h-3 w-3" />
            Back to List
          </Link>
          <div>
            <h1 className="font-serif text-[34px] font-black leading-tight text-[#171111]">Edit Occasion</h1>
            <p className="mt-1 text-sm font-medium text-[#6c5c58]">Update category details and display settings.</p>
          </div>
        </div>
        ...
        <button
          type="submit"
          disabled={loading}
          className="inline-flex h-12 items-center justify-center gap-3 rounded-lg bg-[#171111] px-7 text-sm font-bold uppercase tracking-wider text-white shadow-[0_14px_28px_rgba(23,17,17,0.22)] transition hover:bg-black disabled:cursor-not-allowed disabled:opacity-60"
        >
          {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
          {loading ? 'Updating...' : 'Update Occasion'}
        </button>
      </div>
